...
const getMobileFromToken = () => {
  ...
  try {
    return JSON.parse(atob(token.split(".")[1])).mobile;
  } catch {
    return "USERID";
  }
};
...
const Menu = () => {
  ...
  const [userId] = useState(getMobileFromToken());
    ...
    <div className="menu-container">
      ...
      <div className="menus">
        ...
        <div className="profile" onClick={handleProfileClick}>
          <div className="avatar">{userId.slice(0, 2)}</div>
          <p className="username">{userId}</p>
        </div>
        <button onClick={handleLogout} style={{ marginLeft: "16px", background: "transparent", border: "1px solid rgb(223, 73, 73)", color: "rgb(223, 73, 73)", borderRadius: "4px", padding: "6px 14px", fontSize: "0.8rem", cursor: "pointer" }}>
          Logout
        </button>
      </div>
    </div>
